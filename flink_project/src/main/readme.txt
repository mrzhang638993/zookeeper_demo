...
flink底层是流式处理引擎的。batch对应的是流的特例的。通过流的batch操作实现批的操作处理实现的。
可以基于如下的操作实现截取的：
1.基于时间段信息进行截取操作实现的：从消息的时间窗口进行数据的统计操作实现的。
2.可以基于消息的数量执行操作的。从计数窗口实现操作管理实现。


水印时间是一种特殊的时间格式的。窗口的计算会根据水印时间来触发窗口的计算操作实现的。增加水印时间可以解决网络延时的计算问题的。


通常使用的是托管状态的，使用的是flink进行自己的管理的。自定义的operator是需要自己进行操作处理的。
checkpoint的操作
1.memorybackend：适用于叫少量数据的存储操作的。比如kafka或者是一次仅仅操作一个记录的函数。生产环境下面用不到的，仅适合开发和测试环境使用的。
2.FsStateBackend:数据保存到文件系统中的。适合用于大状态的，长窗口的，或者是大键值状态的处理任务。适用于高可用方案。
3.RocksDBStateBackend 适用于超大状态的操作的。用户适用自定义的窗口的话，不推荐适用这个的。使用FsStateBackend较为合适的。


flink实现批处理和流处理层的代码统一层面实现操作的，可以使用flink sql的操作实现和管理实现的。可以解决流和批处理的接口
统一操作实现处理的。
任何一门语言都是需要提供sql的api的，这样的话，学习flink的成本就会降低很多的。能够自定义执行优化的。
完全可以使用sql语句操作api操作实现的。可以在sql层面实现批和流的统一操作处理和实现的。flink sql可以在批处理和流处理层面的统一操作和实现的。
