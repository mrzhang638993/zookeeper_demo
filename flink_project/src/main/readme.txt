...
broadcast广播可以从taskmanager的角度获取到共享的数据的，而一般的是只能从task中获取到数据进行操作实现的。这样的话，可以节省内存空间。
...
flink是支持广播的，数据是存在于内存中的。flink的broadcast数据存在的是taskmanager中，在taskmanager中的多个task中是共享数据的。


flink stream 读取csv文件。
flink使用kafka读取数据进行操作。读取kafka的数据。
