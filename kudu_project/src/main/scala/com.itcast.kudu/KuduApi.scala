...
class KuduApi {
  ...
  def createTable(): Unit = {
    ...
    val options: CreateTableOptions = new CreateTableOptions().setRangePartitionColumns(List("key").asJava)
      .setNumReplicas(1)
    //  创建表
    // tablet是最终存储数据的，可以存在多个的。之间可以存在主从的概念的。
    // name: String, schema: Schema, builder: CreateTableOptions
    val tableName = "simple"
    val kudeTable: KuduTable = kuduClient.createTable(tableName, schema, options)
  }
}
