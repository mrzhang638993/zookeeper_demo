...
object RealTimeNewUser {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val groupId="new_user"
    val topicName="new_user"
    val  rt_cv_key="today::new_user::current"
    val rt_cv_offset="today::new_user::offset"
     ...
     *
     * */
    val newUser: DStream[(String, Int)] = message.map(_.value()).map(x => (x, 1))
    val uv: LongAccumulator = spark.sparkContext.longAccumulator("uv")
    val keyInfo="new_user_date_"+DateUtils.getDateStr(new Date().getTime,"yyyy-MM-dd")
