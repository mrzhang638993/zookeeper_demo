...
object KafkaSink {
  ...
  def main(args: Array[String]): Unit = {
    ...
    df.writeStream
      .format("kafka")
      // 指定outputMode的格式为追加的操作的
      .outputMode(OutputMode.Append())
      //.option("path", "D:\\document\\works\\zookeeper-demo\\structure_streaming\\src\\main\\scala\\com\\itcast\\structured\\kafka")
      .option("checkpointLocation", "checkpoint")
      .option("kafka.bootstrap.servers", "node01:9092,node02:9092,node03:9092")
      // 数据落地到kafka3的数据的。
      .option("topic", "streaming-test_2")
      .start()
      .awaitTermination()
  }
}
